...
void RenderMap::DrawBusLabel(const std::string& stop_name, const Svg::Color& color, const std::string& bus) const {
...
RenderRoute::RenderRoute(Svg::Document& doc, const RouteDatabase& data, 
...
void RenderRoute::AddBusLines() const {
	...
	for (size_t i = 0; i < route->size(); ++i) {
		...
		const string& next_sup_stop = (i + 1) >= route->size() ? stop_to : route->at(i + 1).bus_or_stop;
		int curr_idx = 0;
		int next_sup_idx = 0;
		...
		for (int j = 0; j < bus_info.stops.size(); ++j) {
			if (bus_info.stops[j] == curr_stop) {
				curr_idx = j;
				if (curr_idx - stop_count >= 0 && bus_info.stops[curr_idx - stop_count] == next_sup_stop) {
					next_sup_idx = curr_idx - stop_count;
					break;
				}
				if (curr_idx + stop_count < route->size() && bus_info.stops[curr_idx + stop_count] == next_sup_stop) {
					next_sup_idx = curr_idx + stop_count;
					break;
				}
			}
			if (bus_info.stops[j] == next_sup_stop) {
				next_sup_idx = j;
				if (next_sup_idx - stop_count >= 0 && bus_info.stops[next_sup_idx - stop_count] == curr_stop) {
					curr_idx = next_sup_idx - stop_count;
					break;
				}
				if (next_sup_idx + stop_count < route->size() && bus_info.stops[next_sup_idx + stop_count] == curr_stop) {
					curr_idx = next_sup_idx + stop_count;
					break;
				}
			}
...
void RenderRoute::AddBusLabels() const {
	...
	for (size_t i = 0; i < route->size(); ++i) {
		if (route->at(i).edge == EdgeType::WAIT) {
			const string& bus = (i + 1) > route->size() 
				? route->back().bus_or_stop 
			...
			if (i >= 1 
				...
				DrawBusLabel(curr_stop, buses.at(route->at(i - 1).bus_or_stop).color, route->at(i - 1).bus_or_stop);
			}
