...
Svg::Color GetColor(const Json::Node& info)
{
    ...
    try {
        ...
        if (arr.size() == 3) {
            color = Svg::Rgb{
                static_cast<uint8_t>(arr[0].AsDouble()), static_cast<uint8_t>(arr[1].AsDouble()),
                static_cast<uint8_t>(arr[2].AsDouble())};
        }
        else {
            color = Svg::Rgba{
                static_cast<uint8_t>(arr[0].AsDouble()), static_cast<uint8_t>(arr[1].AsDouble()),
                static_cast<uint8_t>(arr[2].AsDouble()), arr[3].AsDouble()};
        }
    }
    return color;
}
...
{
    data_.SetRouteInfo(
        RouteInfo({info.at("bus_wait_time").AsDouble(), info.at("bus_velocity").AsDouble()})
    );
}
...
void Reader::ReadRenderSettings(const map<string, Json::Node>& info)
{
    ...
    map_info.stop_label_offset = {stop_label_offset[0].AsDouble(), stop_label_offset[1].AsDouble()};

    auto bus_label_offset = info.at("bus_label_offset").AsArray();
    map_info.bus_label_offset = {bus_label_offset[0].AsDouble(), bus_label_offset[1].AsDouble()};
