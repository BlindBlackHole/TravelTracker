...
//--Coords
Coords::Coords(const MapInfo& map_info, const Extremes& ex) : map_info(map_info), extremes(ex)
{
...
double Coords::GetZoomCoef() const
{
    ...
    if ((std::abs(extremes.max_lon - extremes.min_lon) < 1e-6)
        && (std::abs(extremes.max_lat - extremes.min_lat) < 1e-6))
    {
        zoom_coef = 0.0;
    }
    ...
    {
        height_zoom_coef
            = (map_info.height - 2.0 * map_info.padding) / (extremes.max_lat - extremes.min_lat);
        zoom_coef = height_zoom_coef;
    }
    else if (!(std::abs(extremes.max_lon - extremes.min_lon) < 1e-6) && (std::abs(extremes.max_lat - extremes.min_lat) < 1e-6))
    {
        width_zoom_coef
            = (map_info.width - 2.0 * map_info.padding) / (extremes.max_lon - extremes.min_lon);
...
unordered_map<string, StopsCoords> CompressedCoords::RecalculateGeographicCoords(
    const unordered_map<string, StopsCoords>& stops_geograf_coords
) const
{
    ...
    for (const auto& [bus_name, bus_info] : buses) {
        ...
        for (const auto& stop : support_stops) {
            ...
            for (size_t i = curr_index + 1; next_index != -1 && i < next_index; ++i) {
                ...
                double x = stops_info.at(stops[curr_index]).longtitude
                         + lon_step * (double)(i - curr_index);
                double lat_step =  // y
                    (stops_info.at(stops[next_index]).latitude
                     - stops_info.at(stops[curr_index]).latitude)
                    / (double)(next_index - curr_index);
                double y = stops_info.at(stops[curr_index]).latitude
                         + lat_step * (double)(i - curr_index);
...
pair<bool, int> CompressedCoords::isNeighbor(
    const InternalCoords& prev_stop, const InternalCoords& curr_stop
) const
{
    bool key = false;
    int index = -1;
...
std::unordered_map<std::string, StopsCoords> CompressedCoords::ComputeCompressedCoords() const
{
    ...
    if (xs.size() == 1) {
        c_coords.insert({
            xs.begin()->stop_name,
            {map_info.padding, map_info.height - map_info.padding}
        });
        return c_coords;
    }
...
CompressedCoords::CompressedCoords(std::shared_ptr<Coords> coords, const RouteDatabase& data)
    : coords(coords), data(data), stops_geograf_coords(data.GetStopsCoords())
{
