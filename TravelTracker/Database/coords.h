...
private:
    ...
    std::unordered_map<std::string, StopsCoords> RecalculateGeographicCoords(
        const std::unordered_map<std::string, StopsCoords>& stops_geograf_coords
    ) const;
    std::pair<std::vector<InternalCoords>, std::vector<InternalCoords>> ComputeDigitalCoords(
    ) const;
