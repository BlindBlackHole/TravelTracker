    ...
    for (const auto& bus : busses_info) {
        ...
        for (size_t i = 0; i < stops.size(); ++i) {
            ...
            for (size_t j = i + 1; j < stops.size(); ++j) {
                ...
                // add edge
                graph.AddEdge(Graph::Edge<Weight>({
                    substop_id, to.id, Weight{EdgeType::BUS, time, bus.first, j - i}
                }));
                ...
                if (!bus.second.circle) {
                    ...
                    graph.AddEdge(Graph::Edge<Weight>({
                        substop_id2, from.id, Weight{EdgeType::BUS, time, bus.first, j - i}
                    }));
                }
            }
        }
...
optional<vector<Weight>> Map::CreateRoute(
