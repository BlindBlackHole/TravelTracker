...
namespace Graph {
    ...
    private:
        ...
        void RelaxRoute(
            VertexId vertex_from, VertexId vertex_to, const RouteInternalData& route_from,
            const RouteInternalData& route_to
        )
        {
            auto& route_relaxing = routes_internal_data_[vertex_from][vertex_to];
            const Weight candidate_weight = route_from.weight.weight + route_to.weight.weight;
            if (!route_relaxing || candidate_weight.weight < route_relaxing->weight.weight) {
                route_relaxing
                    = {candidate_weight,
